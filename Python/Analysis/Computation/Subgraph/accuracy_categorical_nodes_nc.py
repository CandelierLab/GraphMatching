...
zeta = 1
l_nc = np.round(np.geomspace(1, nA-1, 10)).astype(int)
...
for nc in l_nc:
  ...
  fname = project.root + '/Files/Success ratios/nMeas_nodes_nc/ER_nA={:d}_nc={:d}_nRun={:d}.csv'.format(nA, nc, nRun)
  ...
  for n in Nsub:
    ...
    for i in range(nRun):
      ...
      val = np.ceil(np.linspace(1e-10, 1, Net.nNd)*nc).astype('int')
