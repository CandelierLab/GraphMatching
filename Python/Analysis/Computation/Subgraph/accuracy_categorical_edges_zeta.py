...
for zeta in l_zeta:
  ...
  fname = project.root + '/Files/Success ratios/nMeas_edges_zeta/ER_nA={:d}_zeta={:d}_nRun={:d}.csv'.format(nA, zeta, nRun)
  ...
  for n in Nsub:
    ...
    for i in range(nRun):
      ...
      for z in range(zeta):
        ...
        val = np.floor(np.linspace(0, 1, Net.nEd)*nc).astype('int')
